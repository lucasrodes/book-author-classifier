...
all_categ = ['becquer','calderon', 'cervantes', 'garcia', 'lope', 'perez', 'pardo','quevedo']
...
for categ in all_categ:
    ...
    # Read file
    f = io.open('/veu4/usuaris30/speech00/corpus/train/spanishlit_ninc_v'+version+'_nlm/'+categ+'.txt',encoding='utf8')
    g = f.read().lower()
...
print "Metric matrix:"
m = pd.DataFrame(mmatrix).reindex_axis(['Recall', 'Precission', 'F1 score'], axis=1)
print m
f1_scores = mmatrix['F1 score'].values()
print "Accuracy: ", str(sum([k*float(l) for (k,l) in zip(n_categ,f1_scores)])/float(sum(n_categ))*100),'%'
